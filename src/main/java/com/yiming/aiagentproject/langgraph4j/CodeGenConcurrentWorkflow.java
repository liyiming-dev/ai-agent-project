...
import com.yiming.aiagentproject.langgraph4j.node.*;
import com.yiming.aiagentproject.langgraph4j.node.concurrent.*;
...
/**
 * langgraph4j的 paralle branch并发方法
 */
@Slf4j
public class CodeGenConcurrentWorkflow {
    ...
    public WorkflowContext executeWorkflow(String originalPrompt) {
        ...
        ExecutorService pool = ExecutorBuilder.create()
                ...
                .build();
        // ParallelNode 由 langgraph4j 在 image_plan 出多边时自动合成，节点 ID = "__PARALLEL__(image_plan)"
        RunnableConfig runnableConfig = RunnableConfig.builder()
                .addParallelNodeExecutor("__PARALLEL__(image_plan)", pool)
                .build();
        int stepCounter = 1;
        try {
